...
class transactionController extends userController {
    ...
    async transfer(data: TransferType) {
        ...
        try {
            ...
            this.transaction_date = moment().format('YYYY-MM-DD HH:mm:ss');
    ...
    async txnHistory(id: number) {
        try {
            ...
            const txnHistory = await HistoryTransaction.findAll({ attributes: ['amount', 'type', 'transaction_date', 'user_id', 'recipient_id', 'recipient_name', 'status'], where: { user_id: id }, order: [['id', 'DESC']] });
