...
import { createAccount, login } from "../schema/user";
...
userRouter.post("/login", checkSchema(login()), async (req: Request, res: Response) => {
    ...
    const reply = await user.login(req.body);
